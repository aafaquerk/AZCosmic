...
def compute_gaussian_parameters(hist, bin_edges, time_sections=False):
    ...
    # min_fit=peak-3*std
    # print(min_fit)
    # gaussian_fit_range = (hist >= min_fit)&(hist <= max_fit)
    # print(np.sum(gaussian_fit_range))
    # gaussian_fit_bins = bin_centers[gaussian_fit_range]
    # print(gaussian_fit_bins)
    # gaussian_fit_counts = hist[gaussian_fit_range]
    window_size = 7
...
def desmear_original(image_data, desmear_threshold_multiplier, gaussian_sigma, gaussian_mean,method="mad", time_sections=False):
    ...
    if method == "mad":
        # Calculate the median and MAD values globally
        medianval = np.median(image_data)
        madval = np.median(np.abs(image_data - medianval))
    else:
        ysize=20
        xsize=20
        # Get median value for all the pixels using a local neighborhood
        ...
        # Calculate MAD value using a local neighborhood
    ...
    # Calculate the percentage of flagged pixels in the region
    ...
    mask_data = np.zeros(np.shape(image_data), dtype=np.int32)  # Changed to int32
    mask_data[idx] = 1
    # Generate masks for analysis
    new_img_data = np.copy(image_data).astype(np.int32)  # Changed to int32
    ...
    for xi, yi in zip(xind, yind):
        if 1 < xi < mask_data.shape[1] - 1:
            if mask_data[yi, xi] == 1 and mask_data[yi, xi - 1] == 0:
                ...
                xval = xi
                counter = 0
                # Move rightwards to count contiguous flagged pixels
                while xval < mask_data.shape[1] and mask_data[yi, xval] == 1:
                    xval += 1
                    counter += 1
                rpix = counter
                # Update the correction mask
    ...
    # Apply random values to corrected pixels
...
def desmear_image(img_data_dask, desmear_threshold_multiplier, read_noise_std, bias_level, time_sections=False):
    # print("Desmear: threshold", threshold, "read_noise_std", read_noise_std, "bias_level", bias_level)
...
def visualize_results(results, read_noise_std, bias_level, time_sections=False):
    ...
    # axes[0, 2].imshow(corr_mask_data, cmap='gray')
    # axes[0, 2].set_title('Correction Mask')
...
#     # raw_hist = plot_histogram(hist_computed, bin_edges_computed, time_sections=TIME_SECTION)
...
#     raw_hist=plot_histogram(hist_computed, bin_edges_computed, event_threshold,cr_threshold, gaussian_mean, gaussian_stddev, raw_emgain,em_slope,em_intercept,linear_fit_sigma_max,linear_fit_sigma_min,hist_plot_max,hist_plot_min,output_dir,input_basename,time_sections=TIME_SECTION)
#     raw_hist.savefig(f'{output_dir}/{input_basename}_histogram_raw_hist.png')
...
#     cr_corrected_dask_image_data = replace_masked_pixels_with_noise(dask_image_data, final_mask, gaussian_mean, gaussian_stddev, time_sections=TIME_SECTION)

#     hist_computed_corrected, bin_edges_computed_corrected = compute_histogram(cr_corrected_dask_image_data, time_sections=TIME_SECTION)
...
#     hist_fig_corrected = plot_histogram(hist_computed_corrected, bin_edges_computed_corrected, time_sections=TIME_SECTION)
...
#     #desmear_results = desmear_image(cr_corrected_dask_image_data, SMEAR_THRESHOLD, gaussian_stddev_corrected, gaussian_mean_corrected,time_sections=TIME_SECTION)
#     desmear_results = desmear_original(cr_corrected_dask_image_data, SMEAR_THRESHOLD, gaussian_stddev_corrected, gaussian_mean_corrected,method='mad',time_sections=TIME_SECTION)
#     #visualize_results(desmear_results, gaussian_stddev_corrected, gaussian_mean_corrected)
#     desmear_corrected_image = desmear_results['corrected_image']
#     desmear_threshold = desmear_results['desmear_threshold']
#     desmear_corrected_image = da.from_array(desmear_corrected_image, chunks=(1000, 1000))
#     hist_computed_desmear, bin_edges_computed_desmear = compute_histogram(desmear_corrected_image, time_sections=TIME_SECTION)
#     gaussian_mean_desmear, gaussian_stddev_desmear = compute_gaussian_parameters(hist_computed_desmear, bin_edges_computed_desmear, time_sections=TIME_SECTION)
#     hist_fig_desmear = plot_histogram(hist_computed_desmear, bin_edges_computed_desmear, time_sections=TIME_SECTION)
#     hist_fig_desmear.savefig(f'{output_dir}/{input_basename}_histogram_desmear_hist.png')
#     event_threshold_desmear = gaussian_mean_desmear + EVENT_THRESHOLD_SIGMA * gaussian_stddev_desmear
#     binary_image, count_above_threshold = threshold_image_and_count_dask(desmear_corrected_image, event_threshold_desmear)
...
#         f.write(f'# Desmear_threshold: {desmear_threshold}\n')
...
def process_image(image_data, image_header, input_basename, output_dir, cr_threshold_sigma=CR_THRESHOLD_SIGMA, event_threshold_sigma=5.5, linear_fit_sigma_max=100, linear_fit_sigma_min=30, cluster_membership_radius=10, cluster_membership_threshold=3, hist_plot_max=40000, hist_plot_min=6000, hist_bins=2000,chunk_size=500,time_sections=False):
    ...
    #     f.write(f'# Number of unique clusters: {unique_clusters_final-1}\n')
    #     f.write(f'# Hot pixels: {hot_pixels_final}\n')
...
def main():
    input_file = r'/Users/arkhan/Documents/Dark_Results/dark_SPIE_examples/test_data_warm/dark_06302024_g8110_minus95_vss0/image000099.fits'
    input_basename = os.path.splitext(os.path.basename(input_file))[0]
    output_dir = os.path.dirname(input_file)
    os.makedirs(output_dir, exist_ok=True)
    image_data = fits.getdata(input_file)[500:1500, 1250:1750]
